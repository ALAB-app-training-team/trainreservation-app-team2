...
public class ReservedSeatController {
    ...
    @PatchMapping
    public ResponseEntity<Void> updateReservedSeats(@PathVariable("id") UUID reservationId, @AuthenticationPrincipal AccountSessionDto session) {
        UUID accountId = (session != null) ? session.getId() : null;
        reservedSeatService.updateReservedSeats(reservationId, accountId);
